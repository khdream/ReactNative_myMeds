...
export default class vTracking extends React.Component {
    constructor(props){
        super(props);
        ...
        return (
            <View style = {styles.container}>
                ...
                <View style = {styles.header}>
                    <Text style = {{color : '#fff', fontSize : 28, fontWeight : 'bold', marginBottom : 12}}>Tracking</Text>
                    ...
                    <ScrollView style = {{flex : 1, width : '100%'}}>
                        {
                            this.items.map((item, index) => 
                                <MedTrackingItem item = {item} hiddenDate = {false} key = {index} onPress = {() => this.goMedDetail()}/>
                            )
                        }
                        <View style = {{height: 25,}}></View>
                    </ScrollView>
                </View>
                <TabNavBottom sel_index = {1} navigation = {this.props.navigation} onPlusPressed = {() => this.setState({isModal : true})}/>
            </View>
        );
    }
}
